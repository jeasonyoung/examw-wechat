...
public class MessageParseImpl implements IMessageParse {
	private static final Logger logger = Logger.getLogger(MessageParseImpl.class);
	...
	public BaseRespMessage messageHandler(Context context, BaseReqMessage req) {
		...
		switch(req.getMsgType()){
			...
			default:{
				if(logger.isDebugEnabled())logger.debug("消息类型["+req.getMsgType()+"]不可知，不能处理！");
				return null;
			}
		}
	}
	...
	public BaseRespMessage messageHandler(Context context, EventMessage event) {
		...
		switch(event.getEvent()){
			...
			case Context.EVENT_MESSAGE_TYPE_VIEW:{
				if(logger.isDebugEnabled())logger.debug("接收到菜单跳转事件事件");
				//菜单处理。
				ViewEventMessage view = (ViewEventMessage)event;
				if(!context.getLastMenuKey().equalsIgnoreCase(view.getEventKey())){
					 if(logger.isDebugEnabled())logger.debug("菜单值:" + view.getEventKey() + ",与上下文菜单值:"  + context.getLastMenuKey() + ",不一致更新上下文中的菜单值。");
					 context.setLastMenuKey(view.getEventKey());
				}
				return this.handler.handler(context, event.getEvent(), (ViewEventMessage)event);
			}
			default:{
				if(logger.isDebugEnabled())logger.debug("事件消息类型["+ event.getEvent() +"]不可知，不能处理！");
				return null;
			}
		}
	}
